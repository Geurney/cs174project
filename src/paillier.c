...
long unsigned int decrypt(char* ciphertext, int base, paillier_pubkey_t* public_key, paillier_prvkey_t* private_key) {
    paillier_ciphertext_t cipher;
    mpz_init(cipher.c);
    mpz_set_str(cipher.c, ciphertext, base);
    paillier_plaintext_t plain;
    mpz_init(plain.m);
    paillier_dec(&plain, public_key, private_key, &cipher);
    return mpz_get_ui(plain.m);
}
